 ...
 * 
 * @author Federico Giust - Tora Soft
 *
 */
class ShortUrl
{
    ...
    /**
     * Log in table url_log each access to the URL based on id.
     * 
     * Logs the User Agent, User Ip and the Date
     * 
     * @param integer $id the ID of the row to increment
     * @throws PDOException if an error occurs
     */    
    protected function url_log($id){
        $date = date('Y-m-d H:i:s');
        $query = "INSERT INTO url_log (url_id_fk, user_agent, user_ip, log_date) VALUES (:id,:user_agent,:user_ip,:log_date)";
        $stmt = $this->pdo->prepare($query);
